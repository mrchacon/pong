...
var PongCore  = function(game_instance){
  ...
  if(this.server) {
    ...
    this.players = {
      self : new Paddle(this,this.instance.player_host),
      other : new Paddle(this,this.instance.player_client)
    };
  ...
  {
    this.players = {
      self : new Paddle(this),
      other : new Paddle(this)
    };

    //Debugging ghosts, to help visualise things
    this.ghosts = {
      //Our ghost position on the server
      server_pos_self : new Paddle(this),
      //The other players server position as we receive it
      server_pos_other : new Paddle(this),
      //The other players ghost destination position (the lerp)
      pos_other : new Paddle(this)
    };
...
//server side we set the 'PongCore' class to a global type, so that it can use it anywhere.
if( 'undefined' != typeof global ) {
  module.exports = global.PongCore = PongCore;
}
...
PongCore.prototype.pos = function(a) { return {x:a.x,y:a.y}; };
//Add a 2d vector with another one and return the resulting vector
PongCore.prototype.v_add = function(a,b) { return { x:(a.x+b.x).fixed(), y:(a.y+b.y).fixed() }; };
//For the server, we need to cancel the setTimeout that the polyfill creates
PongCore.prototype.stop_update = function() {  window.cancelAnimationFrame( this.updateid );  };
//Simple linear interpolation
PongCore.prototype.lerp = function(p, n, t) { var _t = Number(t); _t = (Math.max(0, Math.min(1, _t))).fixed(); return (p + _t * (n - p)).fixed(); };
//Simple linear interpolation between 2 vectors
PongCore.prototype.v_lerp = function(v,tv,t) { return { x: this.lerp(v.x, tv.x, t), y:this.lerp(v.y, tv.y, t) }; };
...
var Paddle = function( game_instance, player_instance )
{
  ...
  if(player_instance) {
    this.pos = { x:20, y:20 }; //First player
  } else {
    this.pos = { x:700, y:20 }; //Second player
  }
...
Paddle.prototype.draw = function()
{
  ...
  game.ctx.fillRect(this.pos.x - this.size.hx, this.pos.y - this.size.hy, this.size.x, 110);

  //Draw a status update
  game.ctx.fillStyle = this.info_color;
  game.ctx.fillText(this.state, this.pos.x > 360 ? this.pos.x-120 : this.pos.x + 10, this.pos.y + 50);

  /*circle(x, y, 10);
  rect(this.pos.x, HEIGHT-paddleh, paddlew, paddleh);
  ...
  if (x + dx < 0)
    ...
    if (y > paddlex && y < paddlex + paddlew)
      dx = -dx;
    *//*else
...
//Main update loop
PongCore.prototype.update = function(t)
{
...
/**
 * Verifies Paddle collision
 * @param item
 */
PongCore.prototype.check_collision = function( item )
{
  //Top
  ...
  //Bottom
  if(item.pos.y >= item.pos_limits.y_max-90 )
  {
    item.pos.y = item.pos_limits.y_max-90;
  }
...
PongCore.prototype.process_input = function( player )
{
...
/**
 * Will return Paddle direction
 * @param x
 * @param y
 * @returns {{x: *, y: *}}
 */
PongCore.prototype.physics_movement_vector_from_direction = function(x,y)
{
...
/**
 * Will "synchronize" positions on server and client sides
 */
PongCore.prototype.update_physics = function()
{
  if(this.server)
  {
...
/**
 * Update "items" on server view
 */
PongCore.prototype.server_update_physics = function()
{
...
/**
 * Notifies clients of changes on server side and handle lag on movement
 */
PongCore.prototype.server_update = function()
{
...
/**
 * Handle communication of inputs
 * @param client
 * @param input
 * @param input_time
 * @param input_seq
 */
PongCore.prototype.handle_server_input = function(client, input, input_time, input_seq)
{
...
/**
 * takes input from the client and keeps a record, it also sends the input information to the server immediately
 * as it is pressed. It also tags each input with a sequence number.
 * @returns {*}
 */
PongCore.prototype.client_handle_input = function()
{
...
/**
 * Handle our local input prediction by correcting it with the server
 */
PongCore.prototype.client_process_net_prediction_correction = function()
{
  //No updates...
  ...
  if(my_last_input_on_server)
  {
    ...
    if(lastinputseq_index != -1)
    {
      ...
      /**
       * Now we reapply all the inputs that we have locally that the server hasn't yet confirmed.
       * This will 'keep' our position the same, but also confirm the server position at the same time.
       */
      this.client_update_physics();
 ...
 * Handle net updates
 */
PongCore.prototype.client_process_net_updates = function()
{
  //No updates...
  if(!this.server_updates.length) return;
...
PongCore.prototype.client_onserverupdate_recieved = function(data)
{
...
PongCore.prototype.client_update_local_position = function()
{
...
PongCore.prototype.client_update_physics = function()
{
 ...
 * Update client view
 */
PongCore.prototype.client_update = function()
{
...
PongCore.prototype.create_timer = function()
{
...
PongCore.prototype.create_physics_simulation = function()
{
  setInterval(function(){
...
PongCore.prototype.client_create_ping_timer = function()
{
...
PongCore.prototype.client_create_configuration = function()
{
...
PongCore.prototype.client_create_debug_gui = function()
{
...
PongCore.prototype.client_reset_positions = function()
{
  var player_host = this.players.self.host ?  this.players.self : this.players.other;
  var player_client = this.players.self.host ?  this.players.other : this.players.self;
...
PongCore.prototype.client_onreadygame = function(data) {
  ...
  player_host.state = 'First player';
  player_client.state = 'Second player';
...
PongCore.prototype.client_onjoingame = function(data)
{
  //We are not the host
...
PongCore.prototype.client_onhostgame = function(data) {
...
PongCore.prototype.client_onconnected = function(data) {
...
PongCore.prototype.client_on_otherclientcolorchange = function(data)
{
  this.players.other.color = data;
};

PongCore.prototype.client_onping = function(data)
{
  this.net_ping = new Date().getTime() - parseFloat( data );
  this.net_latency = this.net_ping/2;
};

PongCore.prototype.client_onnetmessage = function(data)
{
...
PongCore.prototype.client_ondisconnect = function(data)
{
...
PongCore.prototype.client_connect_to_server = function() {
...
PongCore.prototype.client_refresh_fps = function()
{
...
PongCore.prototype.client_draw_info = function() {
